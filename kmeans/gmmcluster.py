...
def doclustering(mlnfile,evidfile,cratio,outmfile,outefile,outclog):
    ...
    for i,t in enumerate(tables):
        ...
        #KM.fit(features)
        gmm = mixture.GMM(n_components=numclusters[i])
        features_scaled = preprocessing.scale(features)
        #print(features)
        gmm.fit(features_scaled)
        PL = gmm.predict_proba(features_scaled)
        c_dict = defaultdict(list)
        for inst,pl in enumerate(PL):
            for cl,v in enumerate(pl):
                if v > 0.1:
                    c_dict[cl].append((inst,v))
        Gprobs.append(c_dict)
        '''
    ...
    for i,p in enumerate(pvec):
        ...
        if len(pindvec[i])==1:
            ...
            C1 = Gprobs[pindvec[i][0]]
            C2 = Gprobs[pindvec[i][1]]
            for k1 in C1.keys():
                for k2 in C2.keys():
                    ev = 0
                    cf = 0
                    ...
                    for obj1 in C1[k1]:
                        for obj2 in  C2[k2]:
                            if obj2[0] in evidtable[ix][obj1[0]]:
                                ev = ev + 1
                            cf = cf + obj1[1]*obj2[1]
                            tot = tot + 1
                    print("EV="+str(ev/(tot+1)))
                    print("CF="+str(cf/(tot+1)))
            sys.exit(0)
        #ofile1.close()
    ofile.close()
    ...
    ofile.close()
    '''
doclustering(sys.argv[2],sys.argv[3],sys.argv[5],"kmeans/cmln.txt","kmeans/cevid.txt","kmeans/clog.dat")
